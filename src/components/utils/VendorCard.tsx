...
const VendorCard = ({item} :any) => {
  return (
    <Box borderRadius="20px" minW="200px" overflow="hidden" bgColor="#fff" border="1px" borderColor="#777373" shadow="0 4px 6px rgba(0, 0, 0, 0.2)">
      <Image src={item.img} boxSize="210px"/>
      <Stack direction="row" pt={4} color="#000" px="5%">
        <Box>
          <Text fontSize={{base:"",md:"18px",lg:"15px"}} fontWeight="semibold" color="#000">{item.name}</Text>
          <Text fontSize={{base:"",md:"15px",lg:"13px"}} fontWeight="semibold" color="#5D2FE0">{item.category}</Text>
        </Box>
      </Stack>
